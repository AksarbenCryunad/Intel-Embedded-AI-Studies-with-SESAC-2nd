...
# @data:2023/03/11
# @author:aiden
# line detection for autonomous driving
import os
import cv2
import math
import queue
...
class LineDetector(object):
    def __init__(self, color):
        ...
        # ROI for line detection
        if os.environ['DEPTH_CAMERA_TYPE'] == 'ascamera':
            self.rois = ((338, 360, 0, 320, 0.7), (292, 315,
                         0, 320, 0.2), (248, 270, 0, 320, 0.1))
        else:
            self.rois = ((450, 480, 0, 320, 0.7), (390, 480,
                         0, 320, 0.2), (330, 480, 0, 320, 0.1))
        self.weight_sum = 1.0
    ...
    def get_area_max_contour(contours, threshold=100):
        ...
        :return:
        '''
        contour_area = zip(contours, tuple(
            map(lambda c: math.fabs(cv2.contourArea(c)), contours)))
        contour_area = tuple(
            filter(lambda c_a: c_a[1] > threshold, contour_area))
    ...
    def add_vertical_line_far(self, image):
        ...
        if up_p[1] - down_p[1] != 0 and up_p[0] - down_p[0] != 0:
            up_point = (
                int(-down_p[1]/((up_p[1] - down_p[1])/(up_p[0] - down_p[0])) + down_p[0]), 0)
            down_point = (
                int((h - down_p[1])/((up_p[1] - down_p[1])/(up_p[0] - down_p[0])) + down_p[0]), h)
    ...
    def get_binary(self, image):
        ...
        mask = cv2.inRange(img_blur, tuple(lab_data['lab']['Stereo'][self.target_color]['min']), tuple(
            lab_data['lab']['Stereo'][self.target_color]['max']))  # 二值化
        eroded = cv2.erode(mask, self.kernel)  # erode
        dilated = cv2.dilate(eroded, self.kernel)  # dilate
    ...
    def __call__(self, image, result_image):
        ...
        for roi in self.rois:
            ...
            contours = cv2.findContours(
                # find contours
                blob, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)[-2]
            ...
            if max_contour_area is not None:
                ...
                # draw the rectangle composed of the four points - debug code
                ...
                line_center_x, line_center_y = (
                    pt1_x + pt3_x) / 2, (pt1_y + pt3_y) / 2

                # draw the center point - debug code
                """ cv2.circle(result_image, (int(line_center_x),
                           int(line_center_y)), 5, (0, 0, 255), -1)
                cv2.putText(
                    result_image,
                    f"x:{int(line_center_x)}",
                    (int(line_center_x), int(line_center_y) - 2),
    ...
    while running:
        try:
            ...
            if not running:
                ...
                continue
        binary_image = line_detect.get_binary(image)
        cv2.imshow('binary', binary_image)
        img = image.copy()
        y = line_detect.add_horizontal_line(binary_image)
        ...
        result_image, angle, x = line_detect(binary_image, image.copy())
        """ up, down = line_detect.add_vertical_line_far(binary_image)
        # up, down, center = line_detect.add_vertical_line_near(binary_image)
...
if __name__ == '__main__':
    ...
    rclpy.init()
    node = rclpy.create_node('line_detect')
    line_detect = LineDetector('yellow')
